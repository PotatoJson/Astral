...
# It remains 'null' (no transition) until the hook animation is complete.
...
func enter() -> void:
	super()
	# Reset the next state on each new hook.
	next_state = null

	# 1. Stop the player's movement immediately.
	parent.velocity = Vector2.ZERO

	# 2. Play the hook animation.
	...
	animation_player.animation_finished.connect(_on_hook_finished)


# process_physics() runs every physics frame. Since the player is stationary
# during a hook, we only need to check if it's time to transition.
func process_physics(_delta: float) -> State:
	# The state machine will transition as soon as 'next_state' is set.
	return next_state


# This function is called ONLY when the "hook" animation finishes.
func _on_hook_finished() -> void:
	# It's good practice to disconnect a one-shot signal.
	animation_player.animation_finished.disconnect(_on_hook_finished)
...
func exit() -> void:
	...
	if animation_player.is_connected("animation_finished", _on_hook_finished):
		animation_player.animation_finished.disconnect(_on_hook_finished)
